...
// Set up local file persistence path
...
// Premium luxury women's fashion images (from public folder) uploaded by the user
...
function readProducts() {
  try {
    ...
    return raw.map((p: any) => ({
      ...p,
      availableSizes: p.availableSizes ? p.availableSizes.filter((s: string) => s !== "XS") : ["S", "M", "L", "XL"]
    }));
  } catch (e) {
...
app.post("/api/orders", (req, res) => {
  ...
  order.orderNumber = "PRET-" + Math.floor(100000 + Math.random() * 900000);
  order.date = new Date().toISOString();
  order.orderStatus = "Pending Review";
  
  if (order.country === "PK") {
    order.paymentStatus = "50% Advance - Pending Screenshot Review";
  } else {
    order.paymentStatus = "100% Paid (Card/Stripe/PayPal Mock)";
  }
...
// POST order status modification (Admin)
...
app.post("/api/reviews", (req, res) => {
  const { productName, customerName, city, rating, comment } = req.body;
...
async function startServer() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`[PRET STUDIO LONDON] Full-stack Express server running on port ${PORT}`);
  });
}
